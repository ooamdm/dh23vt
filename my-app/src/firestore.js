...
export function fetchDashboardData(db, email, currentProjectKey, onData, onError) {
    // =======================================================================
    // ===== BẮT ĐẦU CHẾ ĐỘ THỬ NGHIỆM =====
    // Gán email của sinh viên bạn muốn giả danh vào đây.
    // Nếu để trống (ví dụ: const TEST_EMAIL_OVERRIDE = "";), ứng dụng sẽ dùng email thật của người đăng nhập.
    const TEST_EMAIL_OVERRIDE = "23129286@st.hcmuaf.edu.vn"; // <-- THAY EMAIL SINH VIÊN CẦN TEST Ở ĐÂY
    // =======================================================================

    const finalEmailForQuery = TEST_EMAIL_OVERRIDE || email;

    if (!db || !finalEmailForQuery) {
        onError(new Error("Database instance or email is missing."));
        return () => {}; 
    }

    console.log(`Đang lấy dữ liệu cho email: ${finalEmailForQuery}`);

    const activitiesCol = collection(db, "studentActivities");
    
    // Khôi phục lại logic lọc theo email
    const q = query(activitiesCol, where("Email", "==", finalEmailForQuery));
