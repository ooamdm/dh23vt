// app/layout.jsx
// Sử dụng 'use client' nếu bạn muốn layout này là Client Component
// và có sử dụng hooks, state, hoặc tương tác trình duyệt.
// Nếu không, nó sẽ mặc định là Server Component.
"use client"; // Đặt nếu cần các tính năng tương tác client-side

import React, { useState, useEffect, useCallback, useMemo } from 'react';

// Import các file CSS toàn cục của bạn
// Giả định bạn đã di chuyển index.css từ 'layouts/' ra thư mục gốc hoặc 'app/'
import './globals.css'; // Đường dẫn tới file CSS toàn cục (ví dụ: app/globals.css)

// Import các hàm xác thực và cơ sở dữ liệu từ các file của bạn
// Đảm bảo đường dẫn import là tương đối hoặc đã được cấu hình alias trong jsconfig.json/tsconfig.json
import { initializeApp, getApps } from 'firebase/app';
import { getAuth, onAuthStateChanged, signInWithCustomToken, signInAnonymously } from 'firebase/auth';
import { getFirestore } from 'firebase/firestore';

// Thay đổi import từ '@/...' sang đường dẫn tương đối hoặc đảm bảo alias đúng
// Nếu bạn chưa cấu hình alias, hãy chỉnh đường dẫn tương đối.
import { firebaseConfigA, firebaseConfigB } from '../services/firebase-configs.js'; // Thay đổi đường dẫn này nếu cần
import { signInForProject, signOutAll } from '../apis/auth.js'; // Thay đổi đường dẫn này nếu cần
import { fetchDashboardData } from '../services/firestore.js'; // Thay đổi đường dẫn này nếu cần

// Import các components
import DashboardHeader from '../components/Dashboard/DashboardHeader.jsx'; // Thay đổi đường dẫn này nếu cần
import LoginPage from '../app/login/page.jsx'; // Giả định LoginPage.jsx được chuyển vào components
...
const __firebase_config = typeof window !== 'undefined' && typeof window.__firebase_config !== 'undefined' ? JSON.parse(window.__firebase_config) : null;
...
export default function RootLayout({ children }) {
    ...
    const [currentProjectKey, setCurrentProjectKey] = useState('projectA');
    ...
    const [showLoginPage, setShowLoginPage] = useState(true); // Control which page to show
    ...
    const handlePostAuthState = useCallback((user, projectKey) => {
        if (user && currentProjectKey === projectKey) {
            ...
            setShowLoginPage(false); // Hide login page if user is signed in
        } else if (!signedInUsers.projectA && !signedInUsers.projectB && isAuthReady) {
            setShowLoginPage(true); // Show login page if no user is signed in and auth is ready
        }
        ...
        <html lang="vi">
            <body>
                <div className="antialiased text-gray-800">
                    ...
                    {showLoginPage ? (
                        <LoginPage onSignIn={() => signInForProject(authInstances[currentProjectKey], currentProjectKey)} />
                    ) : (
                        <div id="dashboard-page">
                            {/* DashboardHeader có thể ở đây vì nó là một phần của layout */}
                            <DashboardHeader
                                user={activeUser}
                            ...
                            <main className="container mx-auto py-4 px-4">
                                {/* children sẽ là nội dung của trang cụ thể (page.jsx) */}
                                {children} 
                            </main>
                        </div>
                    )}
